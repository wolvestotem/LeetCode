...
和3Sum几乎一致的解法，思路上需要注意vector双指针的运用，条件时有序
还有循环中duplicate去重技巧
...
Discussion中贡献了一种各家简洁的记录`路径`和`选择列表`的方法：

在我的方法中有path记录路径，nums和unordered_set记录可选列表，index记录终止条件，是一种非常直观的方法

但是实际上可以直接在nums上操作，用curr标记，左边是已经用过的，右边是没用过的，当curr到达数组末尾时，结束条件触发。这是一种更加指向backtracking本质的方法，同一层决策树之间不会互相影响，可以在原始数据上更改而不会带来重复

### 47 Permutation II

这道题任然使用传统方法，记录路径，可选列表，结束条件，但是重点在于如何防止重复。

```C++
if(i>0 && nums[i]==nums[i-1] && !m.count(i-1))
    continue;
```
经典防止重复格式，意义在于防止重复数字之间的顺序改变，**排列是稳定的！**
